...
model_name2 = "InceptionV4"

model_name3 = "MobileNetV2_x2_0"


model_params = "models_parameters/res_incep_mob"
...
def attack_nontarget_by_PGD(adv_prog, img, pred_label, src_label, out=None):
    ...
    while src_label in pred_label:
        ...
        # 生成对抗样本
        # adv = L_PGD(adv_program=adv_prog, eval_program=double_eval_program, gradients=gradients, o=img,
        #           input_layer=input_layer, output_layer=out, step_size=step, epsilon=eps, iteration=10,
        #           pix_num=224*224*3/40, isTarget=False, target_label=0, use_gpu=use_gpu)
        adv = T_PGD(adv_program=adv_prog, eval_program=double_eval_program, gradients=gradients, o=img,
                    input_layer=input_layer, output_layer=out, step_size=step, epsilon=eps, iteration=10,
                    pix_num=3*224*224/30, t=0.5, isTarget=False, target_label=0, use_gpu=use_gpu)
...
def gen_adv():
    ...
    for filename, label in original_files:
        ...
        Res_result, Inception_result, Mob_result = exe.run(double_eval_program,
        ...
        print("Res result: %d, Inception result: %d, Mobile result: %d" % (r_o_label, i_o_label, m_o_label))

        f.write("原始标签为{0}\n".format(label))
        f.write("Res result: %d, Inception result: %d, Mobile result: %d\n" % (r_o_label, i_o_label, m_o_label))

        if r_o_label == int(label) and i_o_label == int(label) and m_o_label == int(label):
            ...
            adv_img = attack_nontarget_by_PGD(double_adv_program, img, pred_label, label, out=[Res_out, Inception_out, Mob_out])
        ...
        elif r_o_label == int(label) and m_o_label == int(label):  # Inception 预测错误
            ...
            adv_img = attack_nontarget_by_PGD(double_adv_program, img, [r_o_label, 0, m_o_label], label, out=[Res_out, Mob_out])
        ...
        elif r_o_label == int(label) and i_o_label == int(label):  # Mobile 预测错误
            ...
            adv_img = attack_nontarget_by_PGD(double_adv_program, img, [r_o_label, i_o_label, 0], label,
            ...
            print("filename:{}, Mobile failed!, Inception failed!".format(filename))

            Res_ratio = 1.0
            Incep_ratio = 0.0
            Mob_ratio = 0.0

            adv_img = attack_nontarget_by_PGD(double_adv_program, img, [r_o_label, 0, 0], label,
                                              out=[Res_out])
